...
<ParamField body="callback" type="string">
  A publicly accessible URL on your server. Sling sends a `POST` request to this URL when the delivery status changes. Only used when `instant` is `false`. See the [callbacks guide](/v1/guides/callbacks) for payload details.
</ParamField>
...
<ParamField body="fallback_sms" type="boolean">
