...
| RCS | `rcs` | [/send-rcs](/v2/api-reference/send-rcs) | Text, standalone card, carousel; `fallback_sms` and `fallback_sms_sender` for SMS fallback |
| Truecaller | `truecaller` | [/send-truecaller](/v2/api-reference/send-truecaller) | Branded verified messages; `ttl` for message expiry; `fallback_sms` and `fallback_sms_sender` for SMS fallback |
