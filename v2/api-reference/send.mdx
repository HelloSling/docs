...
When `channel` is `truecaller`, you can also use `ttl` to expire undelivered messages after a number of seconds, plus `fallback_sms` and `fallback_sms_sender` to deliver as SMS when Truecaller delivery is not possible (for example, `Sling` while `sender` is `Brand`).
